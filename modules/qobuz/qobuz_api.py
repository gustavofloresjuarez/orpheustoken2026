import time
from utils.utils import hash_string, create_requests_session

class Qobuz:
    def __init__(self, app_id: str, app_secret: str, auth_token: str, exception):
        self.api_base = 'https://www.qobuz.com/api.json/0.2/'
        self.app_id = app_id
        self.app_secret = app_secret
        self.auth_token = auth_token
        self.exception = exception
        self.s = create_requests_session()

    def headers(self): 
        return { 
            'X-Device-Platform': 'android',
            'X-Device-Model': 'Pixel 3',
            'X-Device-Os-Version': '10',
            'X-User-Auth-Token': self.auth_token if self.auth_token else None,
            'X-Device-Manufacturer-Id': '482D8CB7-015D-402F-A93B-5EEF0E0996F3',
            'X-App-Version': '5.16.1.5',
            'User-Agent': 'Dalvik/2.1.0 (Linux; U; Android 10; Pixel 3 Build/QP1A.190711.020))'
                          'QobuzMobileAndroid/5.16.1.5-b21041415'
        }

    def _get(self, url: str, params=None):
        if not params:
            params = {}

        r = self.s.get(f'{self.api_base}{url}', params=params, headers=self.headers())

        if r.status_code not in [200, 201, 202]:
            raise self.exception(r.text)

        return r.json()

    def check_token(self): 
        params = {
            'app_id': self.app_id, 
        }
        signature = self.create_signature('user/get', params)
        params['request_ts'] = signature[0] 
        params['request_sig'] = signature[1]

        r = self._get('user/get', params)

        if r['credential']['parameters']:
            print("Account Region:", r["country"])
        elif not r['credential']['parameters']:
            raise self.exception("Free accounts are not eligible for downloading")
        else:
            raise self.exception('Invalid UserID/Token')


    def create_signature(self, method: str, parameters: dict):
        timestamp = str(int(time.time()))
        to_hash = method.replace('/', '')

        for key in sorted(parameters.keys()):
            if not (key == 'app_id' or key == 'user_auth_token'):
                to_hash += key + parameters[key]

        to_hash += timestamp + self.app_secret
        signature = hash_string(to_hash, 'MD5')
        return timestamp, signature

    def search(self, query_type: str, query: str, limit: int = 10):
        return self._get('catalog/search', {
            'query': query,
            'type': query_type + 's',
            'limit': limit,
            'app_id': self.app_id
        })

    def get_file_url(self, track_id: str, quality_id=27):
        params = {
            'track_id': track_id,
            'format_id': str(quality_id),
            'intent': 'stream',
            'sample': 'false',
            'app_id': self.app_id,
            'user_auth_token': self.auth_token
        }

        signature = self.create_signature('track/getFileUrl', params)
        params['request_ts'] = signature[0]
        params['request_sig'] = signature[1]

        return self._get('track/getFileUrl', params)

    def get_track(self, track_id: str):
        return self._get('track/get',  params={
            'track_id': track_id,
            'app_id': self.app_id
        })

    def get_playlist(self, playlist_id: str):
        offset = 0
        limit = 500 # Qobuz max limit per request
        total_items = []
        playlist_data = None

        while True:
            params = {
                'playlist_id': playlist_id,
                'app_id': self.app_id,
                'limit': str(limit),
                'offset': str(offset),
                'extra': 'tracks,subscribers,focusAll'
            }
            res = self._get('playlist/get', params=params)
            
            # Save the main playlist metadata on the first pass
            if not playlist_data:
                playlist_data = res
            
            # Extract the tracks from this batch
            items = res.get('tracks', {}).get('items', [])
            total_items.extend(items)
            
            # Check if we reached the end of the playlist
            total_available = res.get('tracks', {}).get('total', 0)
            print(f"  -> [Qobuz API] Fetching Playlist {playlist_id} tracks: {len(total_items)} / {total_available}...")
            offset += limit
            
            if offset >= total_available or not items:
                break
        
        # Merge all fetched tracks back into the main object
        playlist_data['tracks']['items'] = total_items
        return playlist_data

    def get_album(self, album_id: str):
        return self._get('album/get',  params={
            'album_id': album_id,
            'app_id': self.app_id,
            'extra': 'albumsFromSameArtist,focusAll'
        })

    def get_artist(self, artist_id: str):
        offset = 0
        limit = 500 # Qobuz max limit per request
        total_items = []
        artist_data = None

        while True:
            params = {
                'artist_id': artist_id,
                'app_id': self.app_id,
                'extra': 'albums,playlists,tracks_appears_on,albums_with_last_release,focusAll',
                'limit': str(limit),
                'offset': str(offset)
            }
            res = self._get('artist/get', params=params)
            
            if not artist_data:
                artist_data = res
            
            items = res.get('albums', {}).get('items', [])
            total_items.extend(items)
            
            # Check if we reached the end
            total_available = res.get('albums', {}).get('total', 0)
            offset += limit            # Check if we reached the end
            total_available = res.get('albums', {}).get('total', 0)
            print(f"  -> [Qobuz API] Fetching Artist {artist_id} albums: {len(total_items)} / {total_available}...")
            offset += limit
            
            if offset >= total_available or not items:
                break
        
        # Merge all fetched items back into the main object
        artist_data['albums']['items'] = total_items
        return artist_data
        
    def get_label(self, label_id: str):
        offset = 0
        limit = 500 # Qobuz max limit per request
        total_items = []
        label_data = None

        while True:
            params = {
                'label_id': label_id,
                'app_id': self.app_id,
                'extra': 'albums,focusAll',
                'limit': str(limit),
                'offset': str(offset)
            }
            res = self._get('label/get', params=params)
            
            if not label_data:
                label_data = res
            
            items = res.get('albums', {}).get('items', [])
            total_items.extend(items)
            
            # Check if we reached the end
            total_available = res.get('albums', {}).get('total', 0)
            print(f"  -> [Qobuz API] Fetching Label {label_id} albums: {len(total_items)} / {total_available}...")
            offset += limit
            
            if offset >= total_available or not items:
                break
        
        # Merge all fetched items back into the main object
        label_data['albums']['items'] = total_items
        return label_data
        
    def get_award(self, award_id: str):
        import time # Ensure this is imported at the top of your file
        offset = 0
        limit = 50 
        total_items = []
        award_data = None

        while True:
            print(f"[DEBUG] Requesting Award {award_id} | offset: {offset} | limit: {limit}", flush=True)
            params = {
                'award_id': award_id,
                'app_id': self.app_id,
                # 🚀 CHANGE: Removed focusAll to stop Qobuz backend timeouts on large collections
                'extra': 'albums', 
                'limit': str(limit),
                'offset': str(offset)
            }
            res = self._get('award/get', params=params)
            
            if not award_data:
                award_data = res
            
            items = res.get('albums', {}).get('items', [])
            total_items.extend(items)
            
            # Check if we reached the end
            total_available = res.get('albums', {}).get('total', 0)
            print(f"  -> [Qobuz API] Fetched Award {award_id} albums: {len(total_items)} / {total_available}...", flush=True)
            offset += limit
            
            if offset >= total_available or not items:
                break
            
            # 🚀 CHANGE: Add a tiny delay to stop rate-limiting during deep pagination
            time.sleep(0.5) 
        
        # Merge all fetched items back into the main object
        award_data['albums']['items'] = total_items
        return award_data
              
                
